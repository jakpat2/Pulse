const express = require('express');
const cors = require('cors');
const { SMTCMonitor } = require('@coooookies/windows-smtc-monitor');
const { fileTypeFromBuffer } = require('file-type');
const { Readable } = require('stream');
const net = require('net');
const log = require('electron-log');

// LOG & ERROR
function sendLogToMain(level, message, component = 'SERVER') {
    process.parentPort.postMessage({ 
        type: 'LOG', 
        level: level, // 'info', 'warn', 'error'
        message: message,
        component: component 
    });
}

// Override console methods to forward to main process
console.log = (...args) => sendLogToMain('info', ...args);
console.error = (...args) => sendLogToMain('error', ...args);
console.info = (...args) => sendLogToMain('info', ...args);
log.variables.process = 'server';

function handleFatalError(error) {
    log.error('FATAL ERROR:', error);
    // Send a message to the Main process instead of trying to quit directly
    process.parentPort.postMessage({ type: 'CRITICAL_ERROR', message: error.message });
}

log.errorHandler.startCatching({
  showDialog: false,
  onError: (error) => {
    log.error('Uncaught Exception:', error);
    handleFatalError(error);
  }
});

const app = express();
app.use(cors());

let currentTrackIdentifier = ""; // Tracks "Title - Artist" to detect changes
let cachedCoverBuffer = null;    // The actual image data

// Declare these globally so they are available to all routes and intervals
let PulseResponse = {
    album: "Unknown",
    album_artist: "Unknown",
    artists: ["Unknown"],
    cover_path: "n/a",
    cover_url: "http://localhost:1608/cover.png",
    duration: 0,
    lyrics: "n/a",
    playback_date: new Date().toISOString().split('T')[0],
    playback_time: new Date().toLocaleTimeString(),
    progress: 0,
    status: "stopped",
    status_id: 0,
    title: "Nothing playing",
    track_number: 0,
    track_total: 0
};

const FALLBACK_IMAGE_B64 = "";
const fallbackBuffer = Buffer.from(FALLBACK_IMAGE_B64, 'base64');

// Limit cache size to 50 entries to prevent memory leaks
const MAX_CACHE_SIZE = 50; 
const searchCache = new Map();

async function validateAndCache(key, buffer) {
    if (!buffer) return null;
    
    // Only cache if the image is reasonable (e.g., under 1MB)
    if (buffer.length > 1024 * 1024) return buffer;
    
    // Verify valid image MIME type
    const type = await fileTypeFromBuffer(buffer);
    if (!type || !type.mime.startsWith('image/')) {
        return null;
    }

    // Keep cache size constrained
    if (searchCache.size >= MAX_CACHE_SIZE) {
        const firstKey = searchCache.keys().next().value;
        searchCache.delete(firstKey);
    }
    
    searchCache.set(key, buffer);
    return buffer;
}

// Status Route
app.get('/', (req, res) => res.json(PulseResponse));

// --- APPLE MUSIC API ---
async function getAppleMusicArtwork(title, artist) {
    const cacheKey = `${title.toLowerCase()}-${artist.toLowerCase()}`;
    
    // Check cache to avoid rate limits
    if (searchCache.has(cacheKey)) return searchCache.get(cacheKey);

    try {
        // Use a clean, URL-encoded search term
        const query = `${title} ${artist}`;
        const url = `https://itunes.apple.com/search?term=${encodeURIComponent(query)}&media=music&entity=song&limit=5`;
        
        const controller = new AbortController();
const timeout = setTimeout(() => controller.abort(), 3000); // 3 second timeout

const response = await fetch(url, { signal: controller.signal });
clearTimeout(timeout);
        const data = await response.json();

        // STRICT VALIDATION: Only accept results where name/artist match the input
        // This avoids the "wrong cover" problem by rejecting fuzzy matches
        const match = data.results?.find(item => {
            const trackNameMatch = item.trackName?.toLowerCase() === title.toLowerCase();
            const artistNameMatch = item.artistName?.toLowerCase() === artist.toLowerCase();
            return trackNameMatch && artistNameMatch;
        });

        if (match && match.artworkUrl100) {
            // Fetch the high-resolution version
            const highResUrl = match.artworkUrl100.replace('100x100bb', '600x600bb');
            const imgRes = await fetch(highResUrl);
            if (!imgRes.ok) return null; // Exit if the image load failed
            const buffer = Buffer.from(await imgRes.arrayBuffer());
            
            await validateAndCache(cacheKey, buffer);
            return buffer;
        }
    } catch (e) {
        console.error("Search failed:", e);
    }

    // Cache null to prevent repeated failed requests
    searchCache.set(cacheKey, null);
    return null;
}

// --- DEEZER API (Fallback) ---
async function getDeezerArtwork(title, artist) {
    try {
        const query = `${artist} ${title}`;
        const url = `https://api.deezer.com/search/track?q=${encodeURIComponent(query)}&limit=1`;
        
        const controller = new AbortController();
const timeout = setTimeout(() => controller.abort(), 3000); // 3 second timeout

const response = await fetch(url, { signal: controller.signal });
clearTimeout(timeout);
        const data = await response.json();

        if (data.data && data.data.length > 0) {
            const track = data.data[0];
            const coverUrl = track.album?.cover_big || track.album?.cover_medium;

            if (coverUrl) {
                const imgRes = await fetch(coverUrl);
                if (!imgRes.ok) return null; // Exit if the image load failed
                return Buffer.from(await imgRes.arrayBuffer());
            }
        }
    } catch (e) {
        console.error("Deezer fallback failed:", e);
    }
    return null;
}

// --- DATA UPDATE LOOP ---
async function updateLoop() {
    try {
        const session = SMTCMonitor.getCurrentMediaSession();
        
        // Handle "No Session"
        if (!session?.media) {
            if (PulseResponse.status !== "stopped") {
                Object.assign(PulseResponse, {
                    status: "stopped",
                    status_id: 0,
                    title: "Nothing playing"
                });
                cachedCoverBuffer = fallbackBuffer;
            }
            return;
        }

        const trackIdentifier = `${session.media.title}-${session.media.artist}`;
        const isPlaying = session.playback.playbackStatus === 4;

        if (trackIdentifier !== currentTrackIdentifier) {
    currentTrackIdentifier = trackIdentifier;
    sendLogToMain('info', `Now playing: ${session.media.title}`, 'POLLER');
            
            // Logic for thumbnail
            const isTooBig = session.media.thumbnail && session.media.thumbnail.length >= 1000000;
            const hasThumbnail = session.media.thumbnail && session.media.thumbnail.length > 5000;

            if (session.media.thumbnail && hasThumbnail && !isTooBig) {
                cachedCoverBuffer = Buffer.from(session.media.thumbnail);
            } else {
                // Fetching from API
                let buffer = await getAppleMusicArtwork(session.media.title, session.media.artist) 
                             || await getDeezerArtwork(session.media.title, session.media.artist);
                
                cachedCoverBuffer = buffer || fallbackBuffer;
            }
        }

        // Update PulseResponse
        Object.assign(PulseResponse, {
            album: session.media.albumTitle || "Unknown",
            album_artist: session.media.artist || "Unknown",
            artists: [session.media.artist || "Unknown"],
            duration: (session.timeline.duration || 0) * 1000,
            progress: (session.timeline.position || 0) * 1000,
            status: isPlaying ? "playing" : "paused",
            status_id: isPlaying ? 1 : 0,
            title: session.media.title || "Unknown"
        });
    } catch (e) {
        log.error("Polling cycle error:", e);
    }
}

async function startPolling() {
    try {
        await updateLoop();
    } catch (err) {
        // Even if updateLoop fails, we want to log it and continue
        console.error("Polling cycle error:", err);
    }
    
    // Always wait 1 second after the work is done before starting again
    setTimeout(startPolling, 1000);
}

// --- ROUTE HANDLER ---
app.get('/cover.png', (req, res) => {
    const buffer = cachedCoverBuffer || fallbackBuffer;
    res.set('Content-Type', 'image/png');
    res.set('Content-Length', buffer.length);
    res.set('Cache-Control', 'no-cache, no-store, must-revalidate');
    res.send(buffer);
});
//

// Helper to check if the port is in use
function isPortTaken(port) {
    return new Promise((resolve) => {
        const tester = net.createServer()
            .once('error', () => resolve(true))  // Port is taken
            .once('listening', () => {
                tester.close(() => resolve(false)); // Port is free
            })
            .listen(port);
    });
}

async function init() {
    // 1. Check Port
    const portTaken = await isPortTaken(1608);
    if (portTaken) {
        process.parentPort.postMessage({ 
            type: 'CRITICAL_ERROR', 
            message: 'Port 1608 is already in use. Please close the other app and restart Pulse.' 
        });
        process.exit(1);
        return;
    }

    // 2. Start Server
    app.listen(1608, () => {
        console.log('Pulse server active on http://localhost:1608/');
        
        // 3. Start Polling only AFTER server is live
        startPolling();
    });
}

// Kick everything off
init().catch((err) => {
    process.parentPort.postMessage({ type: 'CRITICAL_ERROR', message: err.message });
    process.exit(1);
});